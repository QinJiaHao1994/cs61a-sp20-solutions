...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    filtered_paragraphs = list(filter(select, paragraphs))
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i in range(0, len(valid_words)):
        ...
        if is_first and diff <= limit:
            index = i
            limit = diff
            is_first = False
        elif not is_first and diff < limit:
            index = i
            limit = diff
...
def feline_fixes(start, goal, limit):
    ...
    if limit < 0 or start == goal: # Fill in the condition
        ...
        substitute_diff = count + feline_fixes(start[1:], goal[1:], limit - count)
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def fastest_words(game):
    ...
    for word_index in words:
        ...
        for player_num in players:
            curr_time = time(game, player_num, word_index)
