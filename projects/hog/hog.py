...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i < num_rolls:
        ...
        result += curr
        i += 1
    return result if not isZero else 1
    # END PROBLEM 1
...
def free_bacon(score):
    ...
    "*** YOUR CODE HERE ***"
    triple = score ** 3
    triple_arr = list(str(triple))
    res = 0
    for i in range(0, len(triple_arr)):
        res += (-1) ** i * int(triple_arr[i])
    return 1 + abs(res)
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    res = 3 ** (player_score + opponent_score)
    res = str(res)
    return res[0] == res[-1]
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    prev_points0 = 0
    prev_points1 = 0
    def feral(num_rolls, points, prev_points):
        if abs(num_rolls - prev_points) == 2:
            points += 3
        return points
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            points = _points = take_turn(num_rolls, score1, dice)
            if feral_hogs:
                points = feral(num_rolls, points, prev_points0)
                prev_points0 = _points
            score0 += points
        else:
            num_rolls = strategy1(score1, score0)
            points = _points = take_turn(num_rolls, score0, dice)
            if feral_hogs:
                points = feral(num_rolls, points, prev_points1)
                prev_points1 = _points
            score1 += points
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    def say(score0, score1):
        ...
        if(score - s > h):
            h = score - s
            print(h, "point(s)! That's the biggest gain yet for Player", who)
        s = score
        return announce_highest(who, h, s)
    return say
    # END PROBLEM 7
