...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        [opponent.hand.pop(0) for _ in range(0, 3)]
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        def assignCard(card, attack, defense):
            card.attack += attack
            card.defense += defense

        attack = other_card.attack
        defense = other_card.defense
        [assignCard(card, attack, defense) for card in player.deck.cards]
        newCards = []
        oCards = opponent.deck.cards
        for i in range(0, len(oCards)):
            if oCards[i].attack != attack and oCards[i].defense != defense:
                newCards.append(oCards[i])
        opponent.deck.cards = newCards
