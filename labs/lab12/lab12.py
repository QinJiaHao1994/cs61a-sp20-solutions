...
def prune_min(t):
    ...
    if t.is_leaf():
        return
    index = 0 if t.branches[0].label < t.branches[1].label else 1
    prune_min(t.branches[index])
    t.branches = [t.branches[index]]
...
def mapl(lst, fn):
    ...
    "*** YOUR CODE HERE ***"
    def f(first, rest):
        return Link(fn(first), rest)
    return foldr(lst, f, Link.empty)
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if check.to != self.holder or check.deposited:
...
def num_splits(s, d):
    ...
    def ways(s, length):
        ...
        if len(s) == 0:
            return []
        return [[s[0]] + item for item in ways(s[1:], length - 1)] + ways(s[1:], length)

    count = 0
    sum_s = sum(s)
    for i in range(0, (len(s) // 2) + 1):
        for v in ways(s, i):
            if abs(sum_s - sum(v) - sum(v)) <= d:
                count += 1
    return count
