...
def closer_city(lat, lon, city1, city2):
    ...
    minus = distance(city, city1) - distance(city, city2)
    closestCity = city1 if minus < 0 else city2
    return get_name(closestCity)
...
def add_chars(w1, w2):
    ...
    if not w1:
        return w2
    index = w2.index(w1[0])
    return add_chars(w1[1:], w2[:index] + w2[index + 1:])
...
def add_trees(t1, t2):
    ...
    for i in range(0, maxLength):
        ...
        if not b2[i:]:
            b2.append(tree(0))
    return tree(label(t1) + label(t2), [add_trees(v[0], v[1]) for v in zip(b1, b2)])
